...
def _band_bar(row: dict, worst_rank: int) -> str:
    """The band drawn as a range, because a number cannot show its own uncertainty."""
    lo, hi = row["rank_p05"], row["rank_p95"]
    span = max(worst_rank, 1)
    left = 100 * (lo - 1) / span
    width = max(100 * (hi - lo + 1) / span, 1.2)
    return (
        f'<div class="band"><i style="left:{left:.2f}%;width:{width:.2f}%"></i>'
        f'<b style="left:{100 * (row["rank"] - 1) / span:.2f}%"></b></div>'
    )
...
def _rows_html(ranking: Ranking) -> str:
    ...
    for row in ranking.rows:
        flip = (
            '<span class="flip" title="This rank moves by more than '
            f'{COIN_FLIP_SPREAD} positions under small changes in weights">coin flip</span>'
            if row["coin_flip"] else ""
        )
        contributors = "".join(
            f'<li><span>{_e(c["label"])}</span><b>{_e(_fmt(c["value"], c["unit"]))}</b></li>'
            for c in row["contributors"]
        )
        drags = "".join(
            f'<li><span>{_e(d["label"])}</span><b>{_e(_fmt(d["value"], d["unit"]))}</b></li>'
            for d in row["drags"]
        )
        out.append(f"""<article class="place">
  <header>
    <div class="rk">{row["rank"]}</div>
    <div class="nm"><h3>{_e(row["name"])}{flip}</h3>
      <p>score {row["score"]:.3f} · rank {row["rank"]}, but anywhere from
         <b>{row["rank_p05"]}</b> to <b>{row["rank_p95"]}</b> under small changes in weights
         · {_e(_fmt(row["population"]))} people · {row["coverage"]:.0%} of weighted data present</p>
    </div>
  </header>
  {_band_bar(row, worst)}
  <div class="cols">
    <div><h4>Why it is here</h4><ul class="good">{contributors}</ul></div>
    <div><h4>What is wrong with it</h4><ul class="bad">{drags}
      <li class="worst"><span>weakest area</span><b>{_e(row["worst_domain"])}
        ({row["worst_domain_score"]:.2f})</b></li></ul></div>
  </div>
</article>""")
    return "\n".join(out)
...
def render_html(report: Report) -> str:
    ...
    baseline = (
        f'<p class="base">Harrisburg, for comparison: score '
        f'{report.baseline["score"]:.3f}, ranked {report.baseline["rank"]:,} of '
        f'{report.baseline["of"]:,} counties. It is scored but never offered as a '
        f'destination.</p>'
        if report.baseline else ""
    )
    flips = report.counties.coin_flips + report.places.coin_flips

    return f"""<title>Where To Live</title>
<style>
:root {{
  --bg:#fbfaf8; --card:#fff; --ink:#1b1a18; --dim:#6a6660; --line:#e6e2dc;
  --good:#2f6f4e; --bad:#a4442f; --band:#cfe0d4; --mark:#2f6f4e; --warn:#8a6d1f;
}}
@media (prefers-color-scheme: dark) {{
  :root:not([data-theme="light"]) {{
    --bg:#151412; --card:#1e1d1a; --ink:#ece8e1; --dim:#9a948a; --line:#302e2a;
    --good:#7bbd95; --bad:#e08a70; --band:#2c4436; --mark:#7bbd95; --warn:#d3b563;
  }}
}}
...
h2 {{ font-size:1.35rem; margin:2.6em 0 .2em; }}
.lede {{ color:var(--dim); max-width:62ch; }}
.note {{ border-left:3px solid var(--band); padding:.6em 0 .6em 1em; margin:1.6em 0;
  color:var(--dim); max-width:64ch; }}
.place {{ background:var(--card); border:1px solid var(--line); border-radius:10px;
  padding:18px 20px; margin:14px 0; }}
.place header {{ display:flex; gap:16px; align-items:flex-start; }}
.rk {{ font-size:1.6rem; font-weight:700; color:var(--dim); min-width:2.2ch;
  font-variant-numeric:tabular-nums; }}
.nm h3 {{ margin:0; font-size:1.15rem; }}
.nm p {{ margin:.25em 0 0; color:var(--dim); font-size:.86rem; }}
.flip {{ font-size:.68rem; text-transform:uppercase; letter-spacing:.08em; margin-left:.7em;
  color:var(--warn); border:1px solid var(--warn); border-radius:99px; padding:.1em .6em;
  font-family:ui-sans-serif,system-ui,sans-serif; vertical-align:middle; }}
.band {{ position:relative; height:8px; background:var(--line); border-radius:99px;
  margin:14px 0 4px; }}
.band i {{ position:absolute; top:0; height:8px; background:var(--band); border-radius:99px; }}
.band b {{ position:absolute; top:-3px; width:2px; height:14px; background:var(--mark); }}
.cols {{ display:grid; grid-template-columns:1fr 1fr; gap:22px; margin-top:14px; }}
@media (max-width:640px) {{ .cols {{ grid-template-columns:1fr; }} }}
h4 {{ margin:0 0 .4em; font-size:.72rem; text-transform:uppercase; letter-spacing:.09em;
  color:var(--dim); font-family:ui-sans-serif,system-ui,sans-serif; }}
ul {{ list-style:none; margin:0; padding:0; font-size:.9rem; }}
li {{ display:flex; justify-content:space-between; gap:12px; padding:.2em 0;
  border-bottom:1px dotted var(--line); }}
li b {{ font-variant-numeric:tabular-nums; white-space:nowrap; }}
.good b {{ color:var(--good); }} .bad b {{ color:var(--bad); }}
.worst {{ border-bottom:none; color:var(--dim); }}
.side {{ display:grid; grid-template-columns:1fr 1fr; gap:26px; margin-top:1em; }}
@media (max-width:640px) {{ .side {{ grid-template-columns:1fr; }} }}
.base {{ color:var(--dim); font-size:.92rem; }}
.alarm {{ border:2px solid var(--warn); border-radius:8px; padding:1em 1.2em; margin:1.6em 0;
  color:var(--ink); max-width:64ch; }}
.alarm b {{ color:var(--warn); }}
...
<div class="wrap">
<h1>Where to live</h1>
<p class="lede">Counties ranked first on the ~39 things measured at county level, then towns
inside the counties that won. Weights: {_e(report.basis)}.</p>

{'<div class="alarm"><b>These are not your weights.</b> Nobody has answered the questionnaire yet, so every ranking below was produced from the placeholder numbers in <code>config/domains.yaml</code>. This page shows that the machine works. It does not show where to live. Run <code>make questionnaire</code> first.</div>' if report.placeholder else ""}

<div class="note"><b>Read the range, not the rank.</b> Every place below shows where it
lands across {report.draws} runs with the weights nudged around. A place ranked 6th that
moves between 2nd and 40th is not better than one ranked 9th — the two are the same answer.
{flips} of the places shown are wide enough to be called coin flips outright.</div>
...
<h2>Towns inside those counties</h2>
{_rows_html(report.places)}

<h2>What this ranking does not know</h2>
<div class="side">
  <div><h4>Weights used</h4><ul>{weights}</ul></div>
  <div><h4>Warnings</h4><ul>{warnings or "<li>None.</li>"}</ul></div>
</div>

<footer>Generated {_e(report.generated_at)} for {_e(report.person)}. Every number traces to
a checksummed public dataset; no article, listicle or search result is an input. A rank
here is a shortlist for visiting, not a decision.</footer>
</div>"""
